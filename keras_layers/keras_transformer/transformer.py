...
def _wrap_layer(name, input_layer, build_func, dropout_rate=0.0, trainable=True, residual_conn=False):
    """Wrap keras_layers with residual, normalization and dropout.

    :param name: Prefix of names for internal keras_layers.
...
def get_encoders(encoder_num,
    ...
    for i in range(encoder_num):
        last_layer = get_encoder_component(
            ...
            dropout_rate=dropout_rate,
            trainable=trainable if trainable_list is None else trainable_list[i],
            residual_conn=residual_conn
        )
    return last_layer
