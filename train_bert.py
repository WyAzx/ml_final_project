...
def train_ml_all_set():
    ...
    train_text, _, train_label, _, train_aux, _, train_sw, _ = train_test_split(text, label, aux, sample_weight,
                                                                                test_size=0.055, random_state=59)
    train_gen = AllDataGenerator(train_text, train_label, train_aux, train_sw, batch_size=32, max_len=512)
    ...
    model.save('save_models/bert.weights-uncased-decay.h5')
    # print('SAVED')
    # parallel_model.fit_generator(train_gen.__iter__(),
    #                              steps_per_epoch=len(train_gen),
    #                              epochs=1,
